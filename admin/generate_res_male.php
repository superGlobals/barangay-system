...
$id = filter_input(INPUT_GET, 'id', FILTER_VALIDATE_INT);
    ...
    $query = $conn->prepare("SELECT * FROM resident JOIN purok ON purok.purok = resident.purok WHERE purok_id =:id AND gender = 'male' AND delete_status != 'Archive'");
...
@import url('http://fonts.cdnfonts.com/css/bookman-old-style');
 ...
 <main id="main" class="main">
    <div class="pagetitle">
      <h1>Resident Male List in <?= $purok ?></h1>
      <nav>
        <ol class="breadcrumb">
          <li class="breadcrumb-item"><a href="index.html">Home</a></li>
          <li class="breadcrumb-item active">Resident Male List in <?= $purok ?></li>
        </ol>
      </nav>
    </div><!-- End Page Title -->
              ...
              <div class="row mt-2">
                <div class="col-12">
                  ...
                  <table class="table text-center">
                    <p class="mb-0 text-dark text-sm">List of Male Resident in <?=$purok?></p>
                  ...
                  <thead>
                   ...
                   <tbody>
                    ...
                    $query = $conn->prepare("SELECT * FROM resident LEFT JOIN purok ON purok.purok = resident.purok WHERE purok_id =:id AND gender = 'male' AND delete_status != 'Archive'");
                    $query->bindParam('id', $id, PDO::PARAM_INT);
                    $query->execute();
                    $result = $query->fetchAll(PDO::FETCH_ASSOC);
                    foreach($result as $row):
