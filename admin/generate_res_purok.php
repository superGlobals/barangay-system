...
$id = filter_input(INPUT_GET, 'id', FILTER_VALIDATE_INT);
    $query = $conn->prepare("SELECT * FROM resident LEFT JOIN purok ON purok.purok = resident.purok WHERE purok_id =:id AND delete_status != 'Archive'");
    $query->bindParam('id', $id, PDO::PARAM_INT);
    $query->execute();
    $purok = $query->fetch(PDO::FETCH_ASSOC);
...
@import url('http://fonts.cdnfonts.com/css/bookman-old-style');
 ...
 <main id="main" class="main">
    <div class="pagetitle">
      <h1>Resident List in <?= $purok ?></h1>
      <nav>
        <ol class="breadcrumb">
          <li class="breadcrumb-item"><a href="index.html">Home</a></li>
          <li class="breadcrumb-item active">Resident List in <?= $purok ?></li>
        </ol>
      </nav>
    </div><!-- End Page Title -->
              ...
              <div class="row mt-2">
                <div class="col-12">
                  ...
                  <table class="table text-center">
                    <p class="mb-0 text-dark text-sm">List of Resident in <?=$purok?></p>
                    <p class="text-dark font-weight-bold text-sm">Total: <?php echo $count ?></p>
                  <thead>
                    <tr>
                    <!-- <th></th> -->
